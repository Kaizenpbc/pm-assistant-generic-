...
export class DataProviderManager {
    ...
    switch (providerName) {
      case 'openweathermap': {
        ...
        if (provider.isConfigured()) {
          logger.info(`[DataProviderManager] Weather provider: ${provider.name}`);
          return provider;
        }
        logger.warn(
  ...
  async getWeather(lat: number, lon: number, days: number = 7): Promise<WeatherForecast> {
    ...
    try {
      ...
      logger.error(`[DataProviderManager] Weather fetch failed: ${errorMessage}`);
  ...
  clearCache(): void {
    ...
    this.cacheHits = 0;
    this.cacheMisses = 0;
    logger.info('[DataProviderManager] Cache cleared.');
  }
  ...
  pruneExpiredEntries(): number {
    ...
    if (pruned > 0) {
      logger.info(`[DataProviderManager] Pruned ${pruned} expired cache entries.`);
    }
    return pruned;
  }
}
