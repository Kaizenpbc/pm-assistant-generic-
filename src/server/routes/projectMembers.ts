...
export async function projectMemberRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/:projectId/members', {
    ...
    try {
      ...
      const members = await memberService.findByProjectId(projectId);
      return { members };
    } catch (error) {
  ...
  fastify.post('/:projectId/members', {
    ...
    try {
      ...
      const member = await memberService.addMember(projectId, data);
  ...
  fastify.put('/:projectId/members/:memberId', {
    ...
    try {
      ...
      const member = await memberService.updateRole(memberId, data.role);
  ...
  fastify.delete('/:projectId/members/:memberId', {
    ...
    try {
      const { memberId } = request.params as { memberId: string };
      const removed = await memberService.removeMember(memberId);
