...
export function registerApprovalTools(server: McpServer) {
  server.tool('list-change-requests', 'List change requests for a project', {
    ...
    jsonResult(await api.get(`/approvals/change-requests/${projectId}`))
  );
  ...
  server.tool('create-change-request', 'Create a change request', {
    ...
    jsonResult(await api.post(`/approvals/change-requests/${projectId}`, data))
  );
  ...
  server.tool('submit-for-approval', 'Submit a change request for approval', {
    ...
    jsonResult(await api.post(`/approvals/change-requests/${changeRequestId}/submit`))
  );
  ...
  }, async ({ changeRequestId, ...data }) =>
    jsonResult(await api.post(`/approvals/change-requests/${changeRequestId}/action`, data))
  );
}
