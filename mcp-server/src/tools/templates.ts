...
export function registerTemplateTools(server: McpServer) {
  server.tool('list-templates', 'List project templates', {}, async () =>
    jsonResult(await api.get('/templates'))
  );
  ...
  server.tool('apply-template', 'Create a project from a template', {
    ...
    jsonResult(await api.post('/templates/apply', params))
  );

  server.tool('list-workflows', 'List automation workflow rules', {}, async () =>
    jsonResult(await api.get('/workflows'))
  );
  ...
  server.tool('create-workflow', 'Create an automation workflow rule', {
    ...
    jsonResult(await api.post('/workflows', params))
  );
}
